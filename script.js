    ...
    lolhuman.on('group-participants-update', async(chat) => {
        try {
            ...
            if (chat.action == 'add') {
                ...
                welkam = `${ini_user.notify}, Selamat datang di ${group_info.subject}`
            ...
            if (chat.action == 'remove') {
                ...
                removefei = `${ini_user.notify}, Keluar dari ${group_info.subject}`
                await lolhuman.sendMessage(chat.jid, ini_img, MessageType.image, { caption: removefei })
            }
        } catch (e) {
            console.log('Error :', e)
        }
    ...
    lolhuman.on('chat-update', async(lol) => {
        try {
            ...
            switch (command) {
                case 'help':
                    var punya_wa = "0@s.whatsapp.net"
                    var ini_text = "IFeiera X LolHumanAPI"
                    var ini_buffer = await getBuffer("https://i.ibb.co/D9q9vg1/wp8913166-hige-wo-soru-wallpapers.jpg")
                    const ini_csreply = {
                        contextInfo: {
                ...
                case 'otakudesu':
                    ...
                    break
                case 'ipaddress':
                    if (args.length == 0) return reply(`Example: ${prefix + command} 103.163.81.14`)
                    ipaddress = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/ipaddress/${ipaddress}?apikey=${apikey}`)
                    get_result = get_result.result
                    ...
                    ini_txt += `Latitude : ${get_result.lat}\n`
                    ini_txt += `Longitude : ${get_result.lon}\n`
                    ...
                    ini_txt += `ASN : ${get_result.as}\n`
                    ini_txt += `Query : ${get_result.query}`
                    reply(ini_txt)
                    break
                case 'mluser':
                    if (args.length == 0) return reply(`Example: ${prefix + command} 84830127/2169`)
                    mluser = args.join(" ")
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/mobilelegend/${mluser}?apikey=${apikey}`)
                    get_result = get_result.result
                    ini_txt = `❖ Nickname : *${get_result}*\n`
                    ini_txt += `❖ PlayerID : *${mluser}*`
                    reply(ini_txt)
                    break
